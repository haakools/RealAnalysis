...
\end{align*}
contains elements from $Y$: i.e. show that there exists a $\{z_n\}$ such that $|z_n -x_n| < \epsilon$ for 
$\sum_{n=1}^\infty |z_n| <0$. We know that $\sum_{n=1}^\infty |z_n|$ converges if and only if 
$\sum_{n=N}^\infty |z_n|$ converges for all $N\in\mathbb{N}$. 
...
\noindent \\
We have proved that a ball $B(\{x_n\}, \epsilon)$ centered around $\{x_n\}\in X \backslash Y$ contains atleast some of its boundary points. So since $Y$ (the complement of $X\backslash Y$) does \emph{not} contain \emph{all} of its boundary points, it cannot be closed.
...
\end{align*}
contains elements from $X\backslash Y$: i.e. show that there exists a $\{z_n\}$ such that $|z_n -y_n| < \epsilon$ for $\sum_{n=1}|y_n|<\infty$ and $\sum_{n=1}|z_n| = \infty$.
...
We have proved that a ball $B(\{y_n\}, \epsilon)$ centered around $\{y_n\}\in Y$ contains atleast some of its boundary points. So since $Y$ (the complement of $X\backslash Y$) does contain \emph{some} of its boundary points, it cannot be open.
...
\draw [thick,decoration={brace,mirror,raise=2pt},decorate] 
  (axis cs:1.5,	0) --
    node[below=5pt] {\footnotesize{$O_2 = f^{-1}(U_2)$}} 
  (axis cs:2,0);
