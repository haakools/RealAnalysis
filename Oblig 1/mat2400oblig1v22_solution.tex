...
So we need to prove that the ball $B(\{x_n\}; \epsilon)$ contains elements from $Y$, we need to show that there exists a $\{z_n\}$ such that $\sum_{i=1}^\infty |z_n| <0$. We know that $\sum_{i=0}^\infty |x_n| = \infty$.\\

\begin{align*}
	|z_n| - |x_n| &\le |z_n -x_n| <\epsilon \\
	|x_n| - |z_n| &\le |z_n -x_n| <\epsilon \\
\end{align*}
...
\textbf{Proof by the contrapositive}: \\
	Suppose $f:X\to F(X)$ is a surjective (?), continous function and that $f(X)$ is disconnected. Since $f(X)$ is disconnected, ther must be two open subsets $U_1$ and $U_2$ in $f(X)$ such that 
\begin{align*}
	U_1 \cup U_2&= f(X)\\
...
\begin{align*}
	O_1 \cup O_2 = X
\end{align*}
Therefore $f(x)$ is either in $U_1$ or $U_2$. Since $O_1, O_2$ are disjoint, because $x\in O_1 \cap O_2$ then $f(x)$ would be in both $U_1$ \emph{and} $U_2$. Thus, $f(X)$ is disconnected.
